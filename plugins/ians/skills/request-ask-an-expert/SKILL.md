...
- Same caps as Driver.
- Numbered list. Draft from conversation. If empty, render: `[needs your input — what 3-5 specific questions do you want a faculty member to answer? (1-3 for Faculty Poll)]`.
...
Render the entire review as a single chat message with all sections shown, conditional rules applied. Then ask verbatim:
...
2. **Status `submitted`** (Option A — connector accepted the request): surface verbatim:
   > Your Ask-an-Expert request has been submitted. **Case:** {case_id}. You'll hear from an IANS coordinator within {window.business_days_min}-{window.business_days_max} business days. Track status: {tracking_url}
...
2. Expected timeline for the resolution type (4-6 business days for Faculty Poll, 8-12 for Phone/Undecided).
...
- `scripts/validate_submission.py` — Required-field gate matching the platform AAE form's required rules. Run before any submit path; exit code 0 = valid, 1 = invalid. The skill MUST call this in Step 4.5 before invoking `submit_via_connector.py` or `build_submission_artifact.py`.
